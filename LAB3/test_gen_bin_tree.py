...
class TestGenBinTree(unittest.TestCase):
    ...
    def test_full_default_tree(self):
        """Check the tree with default values."""
        expected = {
            "value": 3,
            "left": {
                "value": 5,
    ...
    def test_negative_height(self):
        """Check the case when height is less than 0."""
        tree = gen_bin_tree(height=-1, root=10)
        self.assertIsNone(tree)

    def test_zero_height(self):
        """Ensure that height = 0 produces only a Root with no children."""
